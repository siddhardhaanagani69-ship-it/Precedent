...
def run(ctx, cached: dict | None = None) -> dict:
    ...
    plan = ctx.llm.structured("moderator", Plan,
        ...
        "Write 6 SHORT Reddit search queries seeking first-person OUTCOMES for both options. "
        "Each query is 2-4 plain keywords, the way a person types into search. "
        "Include the distinctive topic keyword in every query. Never use boolean operators, "
        "quotation marks or the word AND: they over-constrain the search and return unrelated threads. "
        "Use varied outcome words across the six, such as regret, glad, worth it, update, years later. "
        "Cover both options: three queries per option. Consequences describe events, not advice. "
